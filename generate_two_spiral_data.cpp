...
void getCommandLine(double*, double*, int, char**);
void printHeader(double, double);
void printSet(double, double);


int main(int argc, char** argv)
{
	double density;        /* Density of the data set to generate */
...
void getCommandLine(double* density, double* maxRadius, int argc, char** argv)
{
	if (argc < 2)
		* density = 0.25;
	else
...
void printHeader(double density, double maxRadius)
{
	time_t genTime;
...
void printSet(double density, double maxRadius)
{
	...
	std::ofstream myfile;
	myfile.open("two_spiral_0.25.csv");
	points = (96 * density);
	for (i = 0; i <= points; i++) {
